...
Group **Modelling Giants**: Anh Vo Nhu Tu · Igor Mamyan · Kavya Kinnera · Mika van Straaten
...
First run only — NLTK data (Route A):
```python
import nltk
...
The API loads only this file at startup — no other notebook objects are available. Inference always calls `model["vectorizer"].transform(...)`, **never** `fit_transform`. For Route C, the wrapper runs the tokenizer and forward pass; see `route-c-bertweet-large.ipynb` for the packaging pattern.
